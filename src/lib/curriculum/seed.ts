...
function seedAll(db: Database.Database): void {
  ...
  const proj_validator = challenge(db, proyectos, 'validador-contrasena', 'Validador de contraseñas', 'Una buena contraseña debe cumplir dos condiciones: ser larga (al menos 6 caracteres) y contener al menos un número. Debes leer input.clave, comprobar ambas condiciones y mostrar "segura" o "débil".', 1)
  variant(db, proj_validator, 'js', 'Reto experto 🧠 Lee input.clave. Si tiene 6+ caracteres Y contiene al menos un dígito (0-9), muestra "segura"; si no, muestra "débil".\n\n👉 String tiene .length para contar caracteres. Recorre la cadena con un bucle y comprueba si algún carácter es un dígito.', 'let s = input.clave\nlet largo = s.length >= 6\nlet tieneNum = false\nfor (let i = 0; i < s.length; i++) {\n  if (s[i] >= "0" && s[i] <= "9") tieneNum = true\n}\nif (largo && tieneNum) print("segura")\nelse print("débil")', ['Comprueba dos cosas: longitud (s.length >= 6) y presencia de número.', 'Para detectar un dígito, recorre la cadena y verifica si cada carácter está entre "0" y "9".', 'Usa && para combinar ambas condiciones en el if final.'])
  variant(db, proj_validator, 'python', 'Reto experto 🧠 Lee input["clave"]. Si tiene 6+ caracteres Y contiene al menos un dígito (0-9), muestra "segura"; si no, muestra "débil".\n\n👉 len(s) cuenta caracteres. any(d.isdigit() for d in s) comprueba si hay algún dígito.', 'clave = input["clave"]\nlargo = len(clave) >= 6\ntiene_num = any(d.isdigit() for d in clave)\nif largo and tiene_num:\n    print("segura")\nelse:\n    print("débil")', ['Comprueba dos cosas: longitud (len(clave) >= 6) y presencia de número.', 'Para detectar un dígito, usa any(d.isdigit() for d in clave) que itera sobre los caracteres.', 'Usa and para combinar ambas condiciones en el if.'])
  tc(db, proj_validator, { clave: 'abc' }, 'débil', 0)
  tc(db, proj_validator, { clave: 'abcdef' }, 'débil', 1)
  tc(db, proj_validator, { clave: 'abc123' }, 'segura', 2)
  tc(db, proj_validator, { clave: 'ab1' }, 'débil', 3)

  const proj_guesser = challenge(db, proyectos, 'adivina-numero', 'Adivina el número', 'Tu programa juega a un juego: conoce el número secreto (input.secreto) y recibe una lista de intentos (input.intentos). Por cada intento, debe decir: si es menor que el secreto, "más alto"; si es mayor, "más bajo"; si es igual, "¡acertaste!".', 2)
  variant(db, proj_guesser, 'js', 'Reto experto 🧠 Lee input.secreto (el número a adivinar) e input.intentos (lista de números). Para cada intento, compara: si es menor, imprime "más alto"; si es mayor, imprime "más bajo"; si es igual, imprime "¡acertaste!".\n\n👉 Recorre la lista con un bucle y compara cada elemento con el secreto.', 'for (let i = 0; i < input.intentos.length; i++) {\n  let g = input.intentos[i]\n  if (g < input.secreto) print("más alto")\n  else if (g > input.secreto) print("más bajo")\n  else print("¡acertaste!")\n}', ['Recorre input.intentos accediendo con índices: input.intentos[i].', 'Compara cada intento con input.secreto: si es menor, pide "más alto".', 'Usa if / else if / else para los tres casos.'])
  variant(db, proj_guesser, 'python', 'Reto experto 🧠 Lee input["secreto"] (el número a adivinar) e input["intentos"] (lista de números). Para cada intento, compara: si es menor, imprime "más alto"; si es mayor, imprime "más bajo"; si es igual, imprime "¡acertaste!".\n\n👉 for x in lista recorre directamente; compara cada x con el secreto.', 'secreto = input["secreto"]\nfor g in input["intentos"]:\n    if g < secreto:\n        print("más alto")\n    elif g > secreto:\n        print("más bajo")\n    else:\n        print("¡acertaste!")', ['for g in input["intentos"] itera sobre cada intento directamente.', 'Compara g con secreto: si es menor, pide "más alto".', 'Usa if / elif / else para los tres casos.'])
